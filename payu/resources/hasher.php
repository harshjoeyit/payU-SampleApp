...
class Hasher
{
  ...
  function generate_hash($params) {
    ...
    {
      $payhash_str = $additional_charge .'|'. $key . '|' . $this->checkNull($txnid) . '|' .$this->checkNull($amount)  . '|' .$this->checkNull($productinfo)  . '|' . $this->checkNull($firstname) . '|' . $this->checkNull($email) . '|' . $this->checkNull($udf1) . '|' . $this->checkNull($udf2) . '|' . $this->checkNull($udf3) . '|' . $this->checkNull($udf4) . '|' . $this->checkNull($udf5) . '||||||' . $salt;
    }
    $paymentHash = strtolower(hash('sha512', $payhash_str));
    
    echo "<br /> PAYMENT STR ".$payhash_str;
    echo "<br /> PAYMENT HASH ".$paymentHash;
  ...
  function validate_hash($params) {
    ...
    echo "<br /> REVERSE PAYMENT STR ". $payhash_str;
    echo "<br /> REVERSE HASH ".$reverse_hash;
    // $arr['payment_hash'] = $paymentHash;
